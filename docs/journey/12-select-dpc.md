...
distance from the nearest data. (`skill`, `cos`, `best`/`final`, `path/net` and the
selection distances are all defined in
[Reference › Metrics](../reference/metrics.md), with what each one is load-bearing
for.)
...
systems' outcomes differ only through _where they operate_: Reacher's anchor
...
_selecting the right data_, not from Algorithm 1's loop. The `iters` row explains
...
measured state. Beyond `n_max=3` it is strictly dominated: slower _and_ worse.
...
_negative skill_: worse than assuming the tip does not move. The nearest data sits
...
_anchor_,** and Select-DPC does not use anchors — it picks Hankel _columns_ out of
a pooled bank, and collection trajectories wander away from their anchor under OU
excitation. So "1.98 rad from an anchor" never said how far the _selected_ data
...
Panel A is the compact form: the two point clouds land on the _same_ validity
