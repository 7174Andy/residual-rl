...
From [arXiv:2603.07395](https://arxiv.org/abs/2603.07395) Appendix D:
...
By collecting one library _starting from each of four orientations_ (the paper's `π/4, 3π/4, 5π/4, 7π/4`), each library's data is concentrated in a roughly π/2-wide heading region around its anchor. At runtime we always have **the right library for the current heading**.
...
So switching makes goal-reaching _possible_ (~39% vs single-library's 0%), but it is far from reliable — the dominant failure is the truncation/spin mode below, not the QP.
